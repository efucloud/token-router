...
const (
	...
	tsType              = "ts_type"
	jsonTag             = "json"
	tsConvertValuesFunc = `convertValues(a: any, class: any, asMap: boolean = false): any {
	if (!a) {
		return a;
	}
	if (a.slice) {
...
func NewTypeScriptify() *TypeScriptify {
...
func (t *TypeScriptify) convertType(depth int, typeOf reflect.Type, customCode map[string]string) (string, error) {
	...
	entityName := t.Prefix + typeOf.Name() + t.Suffix
	t.types = append(t.types, entityName)
	result := ""
	if t.CreateInterface {
		result += fmt.Sprintf("interface %s {\n", entityName)
	} else if t.CreateType {
